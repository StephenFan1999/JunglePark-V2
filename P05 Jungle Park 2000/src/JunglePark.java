...
/**
 * This class extends the PApplet application and creates the background
 * window that the ParkGUI objects are added to
 * 
...
public class JunglePark extends PApplet {
  ...
  /**
   * CallBack method Defines initial environment properties such as screen
   * size and to load background images and fonts as the program starts.
   * Initializes the backgroundImage and listGUI instance fields.
   */
  @Override
  public void setup() {
    ...
    // graphic objects (animals and buttons) that would be drawn on the 
    // display window
    listGUI = new ArrayList<ParkGUI>();

    // Adds 3 Buttons to JunglePark: one to add a Tiger, one to add a Deer,
    // and one to clear the park
  ...
  /**
   * Callback method called in an infinite loop. It draws the Jungle Park's 
   * window display
   */
  @Override
  public void draw() {
  ...
  public void mouseReleased() {
    // traverse listGUI and call mouseReleased() method defined for every 
	// graphic object
    for (int i = 0; i < listGUI.size(); i++)
  ...
  public void keyPressed() {
	// switch block determines which key was pressed and adds/removes an
	// object appropriately depending on the key
    switch (Character.toUpperCase(this.key)) {
      // add new tiger to the Jungle Park
      case 'T':
        listGUI.add(new Tiger(this));
        break;
        
      //add new deer to the Jungle Park
      case 'D':
    	listGUI.add(new Deer(this));
    	break;
    	
      // remove an animal from the Jungle Park if the mouse is over it
      // traverse the listGUI list and consider only animal objects to be
      // removed if any
      case 'R':
        for (int i = 0; i < listGUI.size(); i++) {
          // remove the first animal which the mouse is over it while the r-key
          // is pressed
   ...
   */
  public void clear() {
	// traverses the listGUI list and removes all animals from it
    for (int i = 0; i < listGUI.size(); i++) {
